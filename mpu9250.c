              ...
              int pinCS)
{
  ...
  bus_config.sclk_io_num = pinCLK; // CLK
  bus_config.mosi_io_num = pinMOSI; // MOSI
  bus_config.miso_io_num = pinMISO; // MISO
  ...
  if ( MPU9250_OK )
  {
    ...
    if ( AK8963_OK )
    {
      ...
      accelScale = 4.0f/32768.0f; // setting the accel scale to 4G
      // ACCEL_CONFIG,ACCEL_FS_SEL_2G
      // fWriteSPIdata8bits( ACCEL_CONFIG, ACCEL_FS_SEL_2G );
      // accelScale = 2.0f/32768.0f;
      // // accelScale = G * 2.0f / 32767.5f; // setting the accel scale to 2G
      // set gyro scale
      // GYRO_CONFIG,GYRO_FS_SEL_1000DPS
      // 250.0f/32768.0f;
      fWriteSPIdata8bits( GYRO_CONFIG, GYRO_FS_SEL_500DPS );
      gyroScale = 500.0f/32768.0f;
      // 2000.0f/32768.0f;
      //fWriteSPIdata8bits( GYRO_CONFIG, GYRO_FS_SEL_1000DPS );
      //gyroScale = 1000.0f / 32768.0f;
      // read the AK8963 ASA registers and compute magnetometer scale factors
      //   set accel scale
      fReadAK8963(AK8963_ASA, 3 );
      fReadMPU9250 ( 3 , EXT_SENS_DATA_00 );
      // convert to mG multiply by 10
      magScaleX = ((((float)rxData[0]) - 128.0f) / (256.0f) + 1.0f) * 4912.0f / 32760.0f; // micro Tesla
      magScaleY = ((((float)rxData[1]) - 128.0f) / (256.0f) + 1.0f) * 4912.0f / 32760.0f; // micro Tesla
      magScaleZ = ((((float)rxData[2]) - 128.0f) / (256.0f) + 1.0f) * 4912.0f / 32760.0f; // micro Tesla
...
void updateIMU(TickType_t TimePast)
{
  ...
  _ax = (float)_axcounts * accelScale;
  _ay = (float)_aycounts * accelScale;
  _az = (float)_azcounts * accelScale;
  //
  _gx = (float)_gxcounts * gyroScale;
  _gy = (float)_gycounts * gyroScale;
  _gz = (float)_gzcounts * gyroScale;
  //
  _hx = (float)_hxcounts * magScaleX;
  _hy = (float)_hycounts * magScaleY;
  _hz = (float)_hzcounts * magScaleZ;
  ////
  // Serial.print ( "ax = " );
